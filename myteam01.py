...
def createTeam(firstIndex, secondIndex, isRed,
               first = 'AttackingAgent', second = 'ProtectingAgent'):
  """
  ...
  # assign share object for each agent object inside agents
  share = SharedInfo(agents)
  return agents
##########
# Agents #
##########
...
class ReflexCaptureAgent(CaptureAgent):
  ...
  def registerInitialState(self, gameState):
    ...
    self.share.initialize(gameState)

  # called inside of constructor of SharedInfo
...
class SharedInfo():
  # this method is called in creatTeam(), passing a flag and
  def __init__(self, agents):
    self.agents = agents.copy()
    self.ownIndexes = [agent.index for agent in self.agents]
    # assign this object to agents
    for agent in self.agents:
      agent.registerSharedInfo(self)
  # this method takes a gameState object and register information in gameState to sharedInfo
  ...
  def updateOppLocations(self, gameState, agentIndex):
    ...
    # list of location of our agents
    ...
    for oppIndex, oldLocs in self.oppLocs.items():
      ...
      # this enable shared vision, if seen enemy, assign probablity 1
      if pos is not None:
